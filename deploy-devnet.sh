...
DEVNET_RPC="https://devnet.helius-rpc.com/?api-key=b539e607-6c09-4971-9115-7e8e1befc126"
...
solana config set --url "$DEVNET_RPC" --keypair ~/.config/solana/id.json
...
if [[ -z "${MINT:-}" ]]; then
  MINT=$(spl-token -u "$DEVNET_RPC" create-token --decimals 6 2>&1 | grep "Address:" | awk '{print $2}')
  [[ -n "$MINT" ]] || die "Failed to create test token mint."
  ok "created mint: $MINT"
fi

# Create ATA for deployer and mint supply
spl-token -u "$DEVNET_RPC" create-account "$MINT" &>/dev/null || warn "ATA already exists"
spl-token -u "$DEVNET_RPC" mint "$MINT" 100000000 &>/dev/null
...
NEXT_PUBLIC_RPC_URL=https://devnet.helius-rpc.com/?api-key=b539e607-6c09-4971-9115-7e8e1befc126
